...
fa=a("#yith-wcbep-tags-filter"),Ca=a("#yith-wcbep-regular-price-filter-select"),ma=a("#yith-wcbep-regular-price-filter-value"),Da=a("#yith-wcbep-sale-price-filter-select"),na=a("#yith-wcbep-sale-price-filter-value"),Ea=a("#yith-wcbep-weight-filter-select"),oa=a("#yith-wcbep-weight-filter-value"),cb=a("#yith-wcbep-per-page-filter"),Fa=a("#yith-wcbep-show-variations-filter"),db=a("#yith-wcbep-reset-filters"),eb=a("#yith-wcbep-check-by-filters"),q=a("#yith-wcbep-table-wrap").find(".wp-list-table").first(),
X=a("#yith-wcbep-bulk-edit-btn"),E=a("#yith-wcbep-bulk-editor"),Ga=a("#yith-wcbep-bulk-apply"),fb=a("#yith-wcbep-categories-bulk-select"),Y=a("#yith-wcbep-categories-bulk-chosen"),pa=a(".yith-wcbep-attributes-bulk-chosen"),gb=a("#yith-wcbep-tags-bulk-select"),hb=a("#yith-wcbep-tags-bulk-value"),ib=a("#yith-wcbep-tags-bulk-replace"),Z=a("#yith-wcbep-save"),jb=a("#yith-wcbep-cols-settings-btn"),I=a("#yith-wcbep-columns-settings-wrapper"),kb=a("#yith-wcbep-columns-settings-select-all"),lb=a("#yith-wcbep-columns-settings-unselect-all"),
T=a("#yith-wcbep-importer-wrapper"),mb=a("#yith-wcbep-importer-cancel"),nb=a("#yith-wcbep-importer-save"),ga=a("#yith-wcbep-importer-upload-url"),ob=a("#yith-wcbep-importer-upload-btn"),A=[],aa=[],pb=a("#yith-wcbep-undo"),qb=a("#yith-wcbep-redo"),rb=a("#yith-wcbep-new"),Ha=a("#yith-wcbep-delete"),sb=a("#yith-wcbep-export"),ba=a("#yith-wcbep-import");a("#yith-wcbep-imported-file");var tb=a("#yith-wcbep-export-form"),ub=a("#yith-wcbep-export-form-btn"),vb=a("#yith-wcbep-export-ids"),N=a("#yith-wcbep-message"),
ca=q.width(),wb=a(".yith-wcbep-close"),xb=ca*da.width()/100,qa={message:null,overlayCSS:{background:"#000 url()",opacity:.6},ignoreIfBlocked:!0},yb={message:null,overlayCSS:{background:"#000 url()",opacity:.6,cursor:"default"}},M=function(c,b){if(b==L){var e=R.find("tr"),d="";R.find("tr").each(function(){var b=a(this).find("td.file_name input").val(),c=a(this).find("td.file_url input").val();d+='<input type="hidden" class="yith-wcbep-hidden-downloadable-file" data-file-name="'+b+'" data-file-url="'+
...
ta()},La=function(){V=[];w=[];var c=0;q.find("tbody#the-list > tr").each(function(){var b=a(this).children("td"),e=[a(this).children("th")];if(0<b.length){var d=[!1],h=0;b.each(function(){a(this).data("y",c);a(this).data("x",h);d.push(a(this).html());e.push(a(this));h++});V.push(d);w.push(e)}c++});h=zb()},ha=function(){var c=[];q.find("tbody#the-list > tr").each(function(){var b=a(this).children("td");if(0<b.length){var e=[!1];b.each(function(){var b=a(this).find(".yith-wcbep-select-selected"),c=
...
E.find(".yith-wcbep-image-bulk-reset").trigger("click");var c=extra_bulk.chosen;for(i in c){var b=a("#yith-wcbep-"+c[i]+"-bulk-chosen");b.prop("selectedIndex",-1);b.select2()}Y.prop("selectedIndex",-1);Y.select2();pa.each(function(){a(this).prop("selectedIndex",-1);a(this).select2()})},Bb=function(){ea.find("input.is_resetable").each(function(){a(this).val("")});ea.find("select.is_resetable").each(function(){a(this).prop("selectedIndex",0)});ea.find(".chosen.is_resetable").each(function(){a(this).val("").trigger("change")})},
Cb=function(){if(U)for(var c in w){var b=w[c],e;for(e in b)if(a(b[e])[0]==U[0]&&"undefined"!=typeof w[parseInt(c)+1]){a(w[parseInt(c)+1][e]).trigger("click");v.selectText();return}}},Pa=function(){var c=[];I.find("input[type=checkbox]").each(function(){var b=a(this).data("cols-id");a(this).is(":checked")?q.find(".column-"+b).show():(q.find(".column-"+b).hide(),c.push(b))});Qa||a.ajax({type:"POST",data:{hidden_cols:c,action:"yith_wcbep_save_default_hidden_cols"},url:ajaxurl,success:function(a){}});
Ka()};a.fn.extend({hasClasses:function(c,b){if("OR"==b){for(var e in c)if(a(this).hasClass(c[e]))return!0;return!1}var d=!0;for(e in c)a(this).hasClass(c[e])||(d=!1);return d}});a.fn.selectText=function(){var a=this[0];if(document.body.createTextRange){var b=document.body.createTextRange();b.moveToElementText(a);b.select()}else if(window.getSelection){var e=window.getSelection(),b=document.createRange();b.selectNodeContents(a);e.removeAllRanges();e.addRange(b)}};wb.on("click",function(){a(this).parent().fadeOut()});
a(".yith-wcbep-datepicker").datepicker({dateFormat:"yy/mm/dd"});E.draggable({stop:function(){a(this).css({height:"auto"})}});I.draggable();T.draggable();q.css({height:"auto",width:xb});da.resizable({minHeight:20,maxHeight:20,minWidth:100,handles:"e",resize:function(){var c=ca*a(this).width()/100;q.css({height:"auto",width:c})}});a("#yith-wcbep-bulk-editor-container").tabs();ea.find(".chosen").select2();Ra.select2({width:"100%"});pa.select2();v.offset([0,0]);rb.on("click",function(){var a='<tr class="yith-wcbep-table-new-product">',
...
q.block(qa);Z.prop("disabled",!0);X.prop("disabled",!0);c=Ab();var b=[],e;for(e in A)b.push(c[A[e]]);a.ajax({type:"POST",data:{products_to_delete:b,action:"yith_wcbep_bulk_delete_products"},url:ajaxurl,success:function(b){N.html("<p>"+b+"</p>");b=a('<button type="button" class="notice-dismiss" />');b.appendTo(N);N.fadeIn();b.on("click",function(){N.fadeOut()});ka.trigger("click")}})}});sb.on("click",function(){A=ia();if(!(1>A.length)){var a=ha(),b=[],e;for(e in A)b.push(a[A[e]]);a=JSON.stringify([h,
b]);b=document.createElement("a");b.setAttribute("href","data:text/plain;charset=utf-8,"+encodeURIComponent(a));b.setAttribute("download","yith_exported_products.txt");b.style.display="none";document.body.appendChild(b);b.click();document.body.removeChild(b)}});ub.on("click",function(){A=ia();if(!(1>A.length)){var a=ha(),b=[],e;for(e in A)b.push(parseInt(a[A[e]][h.indexOf("ID")]));a=JSON.stringify(b);vb.val(a);tb.submit()}});pb.on("click",function(){1<x&&(x--,Na(J[x-1]))});qb.on("click",function(){J[x]&&
...
c.parent().find(".yith_wcbep_no_display").hide()});wa.on("click",function(){C(!0);I.fadeOut();Ha.val(ajax_object.delete_txt).data("confirm","no").removeClass("yith-wcbep-confirm-btn")});v.on("click",function(a){a.stopPropagation()}).keypress(function(a){13==a.which&&(C(!0),a.stopPropagation(),setTimeout(Cb,0))});ka.on("click",function(){var c=[];ja.each(function(){var b=a(this).val();"undefined"!=typeof b&&b&&c.push([a(this).data("taxonomy-name"),a(this).val()])});var b={f_title_select:ya.val(),f_title_value:P.val(),
f_sku_select:za.val(),f_sku_value:Q.val(),f_categories:la.val(),f_tags:fa.val(),f_attributes:c,f_reg_price_select:Ca.val(),f_reg_price_value:ma.val(),f_sale_price_select:Da.val(),f_sale_price_value:na.val(),f_weight_select:Ea.val(),f_weight_value:oa.val(),f_per_page:cb.val(),f_show_variations:Fa[0].checked?"yes":"no"};list.update(b)});X.on("click",function(){C(!0);var c=[];q.find("tbody#the-list th.check-column > input:checked").each(function(){c.push(a(this).val())});if(1>c.length)alert(ajax_object.no_product_selected);
else{var b=a(window).scrollTop()+50;E.css({top:b});E.fadeIn();a("#wpwrap").block(yb);A=ia()}});a("#yith-wcbep-bulk-cancel").add(".yith-wcbep-close-bulk-editor").on("click",function(){E.fadeOut();Oa();a("#wpwrap").unblock()});E.keypress(function(a){13==a.which&&Ga.trigger("click")});E.find("textarea").keypress(function(a){13==a.which&&a.stopPropagation()});Ga.on("click",function(){var c=fb.val(),b=null!=Y.val()?Y.val():"",d=[],q=[],u=gb.val(),t=hb.val(),x=ib.val();Y.find("option").each(function(){d.push(a(this).val());
q.push(a(this).text())});for(var m in A){var k=A[m];if(-1<h.indexOf("categories")){var p=a(w[k][h.indexOf("categories")]),g=a.parseJSON(p.find(".yith-wcbep-select-selected").val()),f="",r=[],n;for(n in g)r.push(g[n].toString());g=r.slice();switch(c){case "new":f=b;break;case "add":f=g.slice();for(n in b)-1==f.indexOf(b[n])&&f.push(b[n]);break;case "rem":for(n in f=g.slice(),b)m=f.indexOf(b[n]),-1<m&&f.splice(m,1)}r=f;p.find(".yith-wcbep-select-selected").val("["+r+"]");var l="";for(n in r)l+=q[d.indexOf(r[n])],
n<r.length-1&&(l+=", ");p.find(".yith-wcbep-select-values").html(l)}g=extra_bulk.chosen;for(n in g)if(-1<h.indexOf(g[n])){var p=a(w[k][h.indexOf(g[n])]),r=a.parseJSON(p.find(".yith-wcbep-select-selected").val()),l="",y=[],z=a("#yith-wcbep-"+g[n]+"-bulk-select").val(),f=a("#yith-wcbep-"+g[n]+"-bulk-chosen"),v=f.val();for(n in r)r[n]&&y.push(r[n].toString());r=y.slice();switch(z){case "new":l=v;break;case "add":l=r.slice();for(n in v)-1==l.indexOf(v[n])&&l.push(v[n]);break;case "rem":for(n in l=r.slice(),
v)m=l.indexOf(v[n]),-1<m&&l.splice(m,1)}r=l;p.find(".yith-wcbep-select-selected").val("["+r+"]");l="";for(n in r)y=f.find("option[value="+r[n]+"]").text(),l+=y,n<r.length-1&&(l+=", ");p.find(".yith-wcbep-select-values").html(l)}if(-1<h.indexOf("tags")&&(p=a(w[k][h.indexOf("tags")]),g=V[k][h.indexOf("tags")],f="",(""!=t||"del"==u)&&1>p.find(".not_editable").length)){switch(u){case "new":f=t;break;case "pre":f=t+g;break;case "app":f=g+t;break;case "rep":f=g.replace(new RegExp(t,"g"),x);break;case "del":f=
""}p.html(f)}y="weight height width length stock_quantity download_limit download_expiry regular_price sale_price".split(" ");for(n in y)if(-1<h.indexOf(y[n])&&(p=a(w[k][h.indexOf(y[n])]),l=parseFloat(p.html()),f="",g=a("#yith-wcbep-"+y[n]+"-bulk-select").val(),r=a("#yith-wcbep-"+y[n]+"-bulk-value").val(),(!isNaN(r)&&""!=r||"del"==g)&&1>p.find(".not_editable").length)){switch(g){case "new":f=parseFloat(r);break;case "inc":l=isNaN(l)?0:l;f=l+parseFloat(r);break;case "dec":f=0<l?l-parseFloat(r):"";
break;case "incp":f=0<l?l+l*parseFloat(r)/100:"";break;case "decp":f=0<l?l-l*parseFloat(r)/100:"";break;case "decfr":g=a(w[k][h.indexOf("regular_price")]);l=parseFloat(g.html());isNaN(l)||""==l||(f=l-parseFloat(r));break;case "decpfr":g=a(w[k][h.indexOf("regular_price")]);l=parseFloat(g.html());isNaN(l)||""==l||(f=l-l*parseFloat(r)/100);break;case "del":f=""}""!=f&&(0>f||isNaN(f))&&(f=0);p.html(f)}z=["purchase_note","description","shortdesc"];for(n in z)if(-1<h.indexOf(z[n])&&(p=a(w[k][h.indexOf(z[n])]),
l=p.html(),f="",g=a("#yith-wcbep-"+z[n]+"-bulk-select").val(),r=a("#yith-wcbep-"+z[n]+"-bulk-value").val(),y=a("#yith-wcbep-"+z[n]+"-bulk-replace").val(),""!=r&&1>p.find(".not_editable").length)){switch(g){case "new":f=r;break;case "pre":f=r+l;break;case "app":f=l+r;break;case "rep":f=l.replace(new RegExp(r,"g"),y)}p.html(f)}f="manage_stock stock_status sold_individually featured virtual downloadable enable_reviews".split(" ");for(n in f)if(-1<h.indexOf(f[n])&&(p=a(w[k][h.indexOf(f[n])]),g=a("#yith-wcbep-"+
f[n]+"-bulk-select").val(),1>p.find(".not_editable").length))switch(g){case "yes":p.find(".yith-wcbep-editable-checkbox").prop("checked",!1).trigger("click");break;case "no":p.find(".yith-wcbep-editable-checkbox").prop("checked",!0).trigger("click")}f="tax_status tax_class allow_backorders shipping_class status visibility download_type prod_type".split(" ");f=f.concat(extra_bulk.select);for(n in f)-1<h.indexOf(f[n])&&(p=a(w[k][h.indexOf(f[n])]),g=a("#yith-wcbep-"+f[n]+"-bulk-select").val(),"skip"!=
g&&1>p.find(".not_editable").length&&p.find(".yith-wcbep-editable-select").val(g).trigger("change"));z="button_text product_url slug up_sells cross_sells title sku sale_price_from sale_price_to date".split(" ");z=z.concat(extra_bulk.text);for(n in z)if(-1<h.indexOf(z[n])&&(p=a(w[k][h.indexOf(z[n])]),l=p.html(),f="",g=a("#yith-wcbep-"+z[n]+"-bulk-select").val(),r=a("#yith-wcbep-"+z[n]+"-bulk-value").val(),y=a("#yith-wcbep-"+z[n]+"-bulk-replace").val(),(""!=r||"del"==g)&&1>p.find(".not_editable").length)){switch(g){case "new":f=
r;break;case "pre":f=r+l;break;case "app":f=l+r;break;case "rep":f=l.replace(new RegExp(r,"g"),y);break;case "del":f=""}p.html(f)}if(-1<h.indexOf("image")&&(p=a(w[k][h.indexOf("image")]),l=p.html(),f="",g=a("#yith-wcbep-image-bulk-select").val(),r=a("#yith-wcbep-image-bulk-value").val(),y=a("#yith-wcbep-image-bulk-src").val(),(""!=r||"del"==g)&&1>p.find(".not_editable").length)){switch(g){case "new":f=r;break;case "del":y=f=""}p.find(".yith-wcbep-hidden-image-value").val(f);p.find("img").attr("src",
y)}pa.each(function(){var b=a(this);if(-1<h.indexOf("attr_"+b.data("taxonomy-name"))){var c=a(w[k][h.indexOf("attr_"+b.data("taxonomy-name"))]),d=b.parent().parent().find(".yith-wcbep-attributes-bulk-select").val(),e="",f=a.parseJSON(c.find(".yith-wcbep-select-selected").val()),g=b.val(),n=[],m=[],l;for(l in f)f[l]=""+f[l];b.find("option").each(function(){m.push(a(this).val());n.push(a(this).text())});switch(d){case "new":e=g;break;case "add":e=f.slice();for(l in g)-1==e.indexOf(g[l])&&e.push(g[l]);
break;case "rem":for(l in e=f.slice(),g)b=e.indexOf(g[l]),-1<b&&e.splice(b,1)}0>c.find(".yith-wcbep-attr-is-visible").val()&&0<e.length&&(e=[e[0]]);c.find(".yith-wcbep-select-selected").val("["+e+"]");g="";for(l in e)g+=n[m.indexOf(e[l])],l<e.length-1&&(g+=", ");c.find(".yith-wcbep-select-values").html(g)}})}E.fadeOut();Oa();va(!0);a("#wpwrap").unblock()});db.on("click",function(){Bb()});eb.on("click",function(){var c=[],b=[];ja.each(function(b,d){var e=a(d),f=null!=e.val()?e.val():null,g;for(g in f)f[g]=
parseInt(f[g]);c.push([e.data("taxonomy-name"),f])});W=ha();var d=null!=la.val()?la.val().map(parseInt):null,u=[],v;for(v in fa.val()){var t=fa.find("option[value="+fa.val()[v]+"]").html();u.push(t)}for(var w in W){var t=W[w],m=!0;if(0<P.val().length){var k=t[h.indexOf("title")];1>k.length&&(m=!1);switch(ya.val()){case "cont":0>k.indexOf(P.val())&&(m=!1);break;case "notcont":-1<k.indexOf(P.val())&&(m=!1);break;case "starts":0!=k.indexOf(P.val())&&(m=!1);break;case "ends":k.indexOf(P.val())!=k.length-
